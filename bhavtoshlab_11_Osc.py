...
class SR830LockIn_interface:
    ...
    def __init__(self,adress=0):
        ...
        self.orm= pv.ResourceManager().open_resource(self.ad)
        self.last_sens = None
...
class ITC503_temperature_Controller: 
     ...
     def __init__(self,adress):
         self.orm= pv.ResourceManager().open_resource(adress,write_termination = '\r',read_termination='\r')
         self.ramprate = 0
         self.orm.write('$C3')
...
class Oscilloscope:
     def __init__(self,address):
          ...
          self.system = pv.ResourceManager().open_resource(self.address)
          print(self.system.query('*IDN?'))

     def initialize(self):

          idn_string = self.system.query('*IDN?')
          print(f"Identification string: '{idn_string}'")
          self.system.write("*CLS")
          self.system.write("*RST")

     def get_values(self,filename,source):

          #Press Horizontal key Change it to Normal not in XY and Roll

          self.system.write(':WAVeform:SOURce CHANnel'+source)
          self.system.write(':WAVeform:POINts:MODE RAW')
          self.system.write(':WAVeform:POINts 10240')
          self.system.write(":WAVeform:FORMat WORD")
          self.system.write(":WAVeform:BYTeorder MSBF")
          self.Check = self.system.query(":WAVeform:PREamble?")

          (
          self.wav_form, self.acq_type, self.wfmpts, self.avgcnt, self.x_increment, self.x_origin,
          self.x_reference, self.y_increment, self.y_origin, self.y_reference
          ) = self.Check.split(",")
          
          self.Bytes = self.system.query_binary_values(":WAVeform:DATA?", 'H', is_big_endian = True ,container = np.array)
          self.values = self.Bytes

          self.File = open(filename, "w")
          self.File.write('Time, Temp')

          for i in range(0, len(self.values) - 1):
               time_val = abs(float(self.x_origin)) + abs((i * float(self.x_increment)))
               voltage = ((float(self.values[i]) - float(self.y_reference)) * float(self.y_increment)) + float(self.y_origin)
               self.File.write(f"{time_val:E}, {voltage:f}\n")
